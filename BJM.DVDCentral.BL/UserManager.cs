    ...
    public class UserManager : GenericManager<tblUser>
    {
        ...
        public void Seed()
        {
            ...
            if (users.Count == 0)
            {
                ...
                Insert(new User { UserId = "bfoote", FirstName = "Brian", LastName = "Foote", Password = "maple" });
                Insert(new User { UserId = "kvicchiollo", FirstName = "Ken", LastName = "Vicchiollo", Password = "password" });
            }
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(user.UserId))
                {
                    if (!string.IsNullOrEmpty(user.Password))
                    {
                        ...
                        {
                            tblUser userrow = dc.tblUsers.FirstOrDefault(u => u.UserName == user.UserId);
        ...
        public int Insert(User user, bool rollback = false)
        {
            try
            {
                ...
                using (DVDCentralEntities dc = new DVDCentralEntities(options))
                {
                    ...
                    bool inuse = dc.tblUsers.Any(u => u.UserName.Trim().ToUpper() == user.UserId.Trim().ToUpper());
                    ...
                    else
                    {
                        ...
                        newUser.UserName = user.UserId.Trim();
        ...
        public int Update(User user, bool rollback = false)
        {
            try
            {
                ...
                {
                    // Check if username already exists - do not allow ....
                    tblUser existingUser = dc.tblUsers.Where(u => u.UserName.Trim().ToUpper() == user.UserId.Trim().ToUpper()).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        if (upDateRow != null)
                        {
                            ...
                            upDateRow.UserName = user.UserId.Trim();
